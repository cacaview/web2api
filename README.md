...
| 版本 | 日期 | 重点内容 |
|------|------|---------|
| **V1.3** | 2026.06 | ✨ 异步URL捕获 / 流量拦截 / 长会话熔断 / 官方配额引擎 / **Web UI 管理面板** / **Gemini Guest Mode** / **反自动化检测** |
...
| GET | `/` | **Web UI 管理面板** |
| GET | `/health` | 健康检查 |
| POST | `/v1/chat/completions` | OpenAI 兼容接口 (支持 `stream=true`) |
| POST | `/api/v1/message` | 原生消息接口 |
| GET | `/api/v1/stats` | 网关统计信息 |
| GET | `/api/v1/admin/accounts` | 账号池状态 |
